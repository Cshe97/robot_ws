    ...
    try:   
        while rclpy.ok():
            ...
            # [320, 240]
                ...
                ret,thresh1 = cv2.threshold(blur, 100, 255, cv2.THRESH_BINARY_INV) #명암 75 이하 255, 이상 0으로 이진화
                ...
                contours = sorted(contours, key=lambda x: cv2.contourArea(x), reverse=True)
                   #윤곽선 개수 확인
                num_contours = len(contours)
                print(f"contours 배열의 인덱스 개수: {num_contours}")
                if finding == 0:
                    if len(contours) > 0:
...
#------------------------------------------------------------------------------------
    ...
    #contour 2개이상
                            if len(contours)>2:   
                                #모든 contour 분홍색으로 표시
                                for contour in contours:
                                    cv2.drawContours(copy_img, contour, -1, (255,0,255),1)
                                    GetGreen(frame)
                                cv2.imshow('copy_img(2<)', copy_img)
                                go()
                                pub.publish(tw)
    #contour 1개   
                    ...
                    if isGreen == 1:
                        ...
                        while timer.cnt_sec < duration: 
                            rclpy.spin_once(timer, timeout_sec = 0.1)
